...
// ── Config ────────────────────────────────────────────────────────
var SHEET_ID = "1YNoQU4uMWbv5gTkgHjPF12JerVzvYRx7ez0Cz39Nyzs"; // TEST sheet — "Copy of Customer Success Tracking Sheet 2"
...
//   90 = TESTING already-onboarded people whose demo is several weeks old. [CURRENT]
//   30 = PRODUCTION (fresh onboards only) — faster, fewer pages to scan.
var FATHOM_SCAN_DAYS_BACK = 90;
...
// ── Web app entry point ───────────────────────────────────────────
// Routes based on the ?app= URL parameter — lupe/rene serve their own
// app directly (so a bookmarked/shared link still works exactly as
// before), no parameter at all (or an unrecognized one) shows the
// landing page letting someone choose between them.
function doGet(e) {
  var app = (e && e.parameter && e.parameter.app) || "";
  // Every page needs this same real, absolute URL — not just Landing.
  // Any link back to the chooser (from inside Lupe or Rene) hits the
  // identical relative-link bug otherwise: a bare href resolves against
  // the googleusercontent.com sandbox domain the page is actually served
  // from, not the real public /exec URL, and goes nowhere useful.
  var baseUrl = ScriptApp.getService().getUrl();
  // Carries a value typed into the shared landing page's LUPE or RENE
  // box straight through to that tool's own page, so it can auto-run
  // the pull on load instead of making the person retype it there.
  var initialQuery = (e && e.parameter && e.parameter.q) || "";

  if (app === "lupe") {
    var lupeTemplate = HtmlService.createTemplateFromFile("Index");
    lupeTemplate.baseUrl = baseUrl;
    lupeTemplate.initialQuery = initialQuery;
    return lupeTemplate.evaluate()
      .setTitle("LUPE")
      .addMetaTag("viewport", "width=device-width, initial-scale=1");
  }

  if (app === "rene") {
    var reneTemplate = HtmlService.createTemplateFromFile("ReneIndex");
    reneTemplate.baseUrl = baseUrl;
    reneTemplate.initialQuery = initialQuery;
    return reneTemplate.evaluate()
      .setTitle("RENE")
      .addMetaTag("viewport", "width=device-width, initial-scale=1");
  }

  var landingTemplate = HtmlService.createTemplateFromFile("Landing");
  landingTemplate.baseUrl = baseUrl;
  return landingTemplate.evaluate()
    .setTitle("CRE OneSource Tools")
    .addMetaTag("viewport", "width=device-width, initial-scale=1");
}
...
function getMeetingRepsFromHubSpot(contactId, contactFullName) {
  var out = { salesperson: "", onboarder: "", onboardingScheduledFuture: false, onboardingScheduledDateIso: "",
              ...
              candidateLastOnboardingDateIso: "", knownPastTrainingCount: 0, onboardingMeetingId: "" };
  ...
  meetings.forEach(function(mm) {
    ...
    var obj = { _id: mm.id, meeting_title: p.hs_meeting_title || "", _start: p.hs_meeting_start_time || "", _owner: p.hubspot_owner_id || "", _notes: (p.hs_internal_meeting_notes || "").replace(/<[^>]*>/g, "").trim() };
  ...
  if (trainings.length) {
    ...
    out.onboardingMeetingId = trainingPick ? trainingPick.meeting._id : "";
    ...
    if (!trainingIsFuture) {
      ...
      if (pastTrainings.length) {
        ...
        // in the CURRENT cycle — used downstream as the actual TARGET for
        // the Fathom search, instead of a flat page-count tolerance
        // that's wrong for every contact in one direction or the other
        // (too aggressive for a simple 1-call contact, still not enough
        // for a contact with several scattered real calls). Knowing
        // "there should be exactly N" lets the search stop immediately
        // and confidently once N is found, or keep going with real
        // justification when it isn't — rather than guessing either way.
      ...
      //
      // This check now anchors off the CURRENT CYCLE's own earliest
      // meeting (recentCycleAnchor), not the raw absolute-earliest
      // training call — otherwise it would itself go chasing an owner
      // match against a stale, years-old meeting, the exact same failure
      // mode the stale-history guard above exists to prevent.
...
// ── Called from the frontend: add the Fathom recording link to the
// onboarding meeting's own "Internal notes" field (hs_internal_meeting_notes) ──
// Genuinely non-blocking by design, per direct instruction — if no
// meeting was matched (different email, meeting never got logged in
// HubSpot, etc.) or Fathom never resolved a recording, this returns a
// clear "skipped" result rather than an error, and every other action in
// the assign-tasks flow keeps working normally regardless.
//
// IMPORTANT: a PATCH to hs_internal_meeting_notes REPLACES the entire
// field — it does not append. This function always reads the current
// value first and writes back the combined result, so nothing anyone
// else already wrote there is ever silently destroyed. In practice this
// field is normally empty for onboarding meetings (unlike the detailed
// post-demo notes some reps write for demo calls, a separate meeting
// type entirely), but the safe read-first approach costs nothing and
// protects against the rare exception.
//
// Also guards against inserting a duplicate link if this ever runs more
// than once against the same meeting (e.g. "Assign tasks" clicked twice).
function addFathomLinkToMeetingNotes(meetingId, fathomUrl) {
  if (!meetingId) return { ok: false, skipped: true, error: "No matching HubSpot meeting was found — skipped." };
  if (!fathomUrl) return { ok: false, skipped: true, error: "No Fathom recording URL was resolved — skipped." };

  try {
    var current = hsFetch("/crm/v3/objects/meetings/" + meetingId + "?properties=hs_internal_meeting_notes", "get");
    var existingNotes = (current && current.properties && current.properties.hs_internal_meeting_notes) || "";

    if (existingNotes.indexOf(fathomUrl) !== -1) {
      return { ok: true, alreadyPresent: true };
    }

    var linkHtml = '<p><a href="' + fathomUrl + '">▶ Watch the onboarding recording</a></p>';
    var combined = linkHtml + existingNotes;

    hsFetch("/crm/v3/objects/meetings/" + meetingId, "patch", {
      properties: { hs_internal_meeting_notes: combined }
    });
    return { ok: true };
  } catch (e) {
    return { ok: false, error: e.message || String(e) };
...
function getFathomMeetingRange(personName, personEmail, personCompany, signupDateIso, knownOnboardingDateIso, knownLastOnboardingDateIso, dealCreatedDateIso, knownPastTrainingCount, pastDemoDateIso, suggestedUserTypeFromDealSize, likelyRepNameHint, pastDemoStandIn) {
  ...
  var summarize = function(items, sourceLabel) {
    ...
    if (!onboardings.length) {
      // ── Demo-stand-in promotion ──────────────────────────────────
      // When HubSpot's own Meetings tab has NO dedicated training call at
      // all, a demo-titled meeting is already standing in as the real
      // onboarding session (see onboarderFromPastDemo in
      // getMeetingRepsFromHubSpot, passed in here as pastDemoStandIn) —
      // but titles like "OneSource Meeting" are HARD-excluded from ever
      // counting as onboarding above (see isOnboardingMeeting), so this
      // branch would otherwise ALWAYS report zero onboarding calls for
      // that exact scenario, no matter how long or wide the search runs.
      // Confirmed real case: Cannon Thornhill's only Fathom-visible call
      // is titled "CRE OneSource Meeting w/ Cannon Thornhill" —
      // classified as a demo, never onboarding — which burned the full
      // 240s search budget (four separate Fathom rate-limit backoffs)
      // chasing a result that could never come, even though the actual
      // answer (who hosted that exact call) was sitting in THIS tier's
      // own already-fetched results the whole time.
      //
      // Only engages when the caller explicitly signals this scenario
      // (pastDemoStandIn === true); every other contact's classify
      // behavior here is completely unaffected. Reuses demoPick (already
      // computed just above) rather than re-selecting a host, so the
      // same-meeting salesperson retraction below mirrors the identical
      // rule already applied on the HubSpot side.
      if (pastDemoStandIn && demoPick) {
        ...
        // The same meeting can't independently prove BOTH roles — retract
        // salesperson if it came from this exact meeting, mirroring the
        // identical rule in getMeetingRepsFromHubSpot.
      ...
      return { firstDate: "", firstOnboarder: "", lastDate: "", lastOnboarder: "", totalCount: 0, salesperson: salesperson,
               demoTitle: demoTitle, demoSummary: demoSummary, onboardingTitle: "", onboardingSummary: "", onboardingShareUrl: "", suggestedUserType: suggestedUserTypeFromDealSize || "" };
    }
    ...
    return {
      ...
      onboardingShareUrl: onboardingEvidence.share_url || "",
  ...
  try {
    for (var repIdx = 0; repIdx < repEmails.length && !stopScanning; repIdx++) {
      ...
      while (true) {
        ...
        // Same demo-stand-in reasoning as the tiers above: if this
        // contact's only real meeting is a demo-titled call standing in
        // for onboarding (pastDemoStandIn), isOnboardingMeeting can NEVER
        // return true for it — hasOnboarding would stay false forever,
        // and this scan would burn its entire request/time budget for a
        // result that could never come, even once the exact meeting has
        // already been found via hasDemo. Stop as soon as a demo shows up
        // in that specific scenario, exactly mirroring the promotion
        // logic in summarize() above.
...
function pullContactData(input) {
  ...
  try {
    ...
    // Smart resolution: company falls back to the PRIMARY associated
    // company record when the flat property is blank; name strips out a
    // company that leaked into the name fields, or reconstructs from a
    // dotted email local-part.
    ...
    return {
      ...
      fathomContext: { fullName: fullName, email: contact.email, organization: organization, dealSource: deal.source || "",
                        ...
                        onboardingMeetingId: hsReps.onboardingMeetingId || "" }
    };
  } catch (e) {
...
function resolveFathomAndFinalize(contactId, ctx) {
  ...
  return {
    ...
    trace: TRACE,
    // Both needed for the automatic "add Fathom link to meeting notes"
    // step — the meeting ID comes from HubSpot's own Meetings tab
    // (captured earlier in getMeetingRepsFromHubSpot), the share URL
    // comes from whichever Fathom recording was actually matched as
    // onboarding evidence. Either can be blank (no meeting matched, or
    // Fathom found nothing) — that's expected and handled as a graceful
    // skip on the frontend, not an error.
    onboardingMeetingId: ctx.onboardingMeetingId || "",
    fathomRecordingUrl: fathom.onboardingShareUrl || "",
    aiCheckContext: {
